...
export const informationSchema = object({
    ...
    politica: optional(
        string([
            minLength(100, "Este campo permite como minímo 100 carácteres"),
        ])
    ),
    enabled: optional(boolean("Este campo solo permite verdadero o falso")),
});

export type InformationSchema = Input<typeof informationSchema>;
export type InformationSchemaFromServe = InformationSchema & {
    createdAt: Date;
    updatedAt: Date;
};
